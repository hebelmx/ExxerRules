Properties {
    ...
    $solutionFileCS = "$rootDir\ExxeRules.CSharp.sln"
    $solutionFileVB = "$rootDir\ExxeRules.VisualBasic.sln"
    $srcDir = "$rootDir\src"
    $testDir = "$rootDir\test"
    ...
    $nuspecPathCS = "$rootDir\src\CSharp\ExxeRules\ExxeRules.nuspec"
    $nuspecPathVB = "$rootDir\src\VisualBasic\ExxeRules\ExxeRules.nuspec"
    ...
    $nupkgPathCS = "$rootDir\src\CSharp\ExxeRules.CSharp.{0}.nupkg"
    $nupkgPathVB = "$rootDir\src\VisualBasic\ExxeRules.VisualBasic.{0}.nupkg"
    ...
    $dllCS = "ExxeRules.CSharp.dll"
    $dllVB = "ExxeRules.VisualBasic.dll"
    $dllCommon = "ExxeRules.Common.dll"
    $testDllCS = "ExxeRules.Test.CSharp.dll"
    $testDllVB = "ExxeRules.Test.VisualBasic.dll"
    $testDirCS = "$testDir\CSharp\ExxeRules.Test\bin\Release"
    $testDirVB = "$testDir\VisualBasic\ExxeRules.Test\bin\Release"
    $projectDirVB = "$srcDir\VisualBasic\ExxeRules"
    $projectFileVB = "$projectDirVB\ExxeRules.vbproj"
    $releaseDirVB = "$projectDirVB\bin\Release"
    $projectDirCS = "$srcDir\CSharp\ExxeRules"
    $projectFileCS = "$projectDirCS\ExxeRules.csproj"
    ...
    $tempDir = Join-Path "$([System.IO.Path]::GetTempPath())" "ExxeRules"
...
Task Update-ChangeLog {
    ...
    return
    Exec {
        github_changelog_generator ExxeRules/ExxeRules --no-pull-requests --no-issues-wo-labels --exclude-labels "Can't repro","update readme",decision,docs,duplicate,question,invalid,wontfix,Duplicate,Question,Invalid,Wontfix  -t $token
    }
}
...
function RunTestWithCoverage($fullTestDllPaths) {
    ...
    $arguments = '-register:user', "`"-target:$xunitConsoleExe`"", "`"-targetargs:$targetArgs $appVeyor -noshadow -parallel none -nologo`"", "`"-filter:+[ExxeRules*]* -[ExxeRules.Test*]*`"", "`"-output:$outputXml`"", '-coverbytest:*.Test.*.dll', '-log:All', '-returntargetcode'
